...
func insertProducts(db *gorm.DB){
	...
	/*
		TYPE OF PRODUCTS
	 */
	db.Exec(query,1,"AGUARDIENTE ANTIOQUEÑO","","CO","aguardiente")
	db.Exec(query,2,"AJIACO CONGELADO","","CO","ajiaco")
	db.Exec(query,3,"AREPAS","","CO","arepa")
	db.Exec(query,4,"AREQUIPE","","CO", "arequipe")
	db.Exec(query,5,"BOCADILLO","","CO","bocadillo")
	db.Exec(query,6,"BOM BOM BUM","","CO","bombombum")
	db.Exec(query,7,"CAFECITO","","CO","cafe")
	db.Exec(query,8,"CHOCOLATINA JET","","CO","chocolatinajet")
	db.Exec(query,9,"CHORIZO","","CO","chorizo")
	db.Exec(query,10,"CROQUETAS DE YUCA","","CO","croquetasyuca")
	db.Exec(query,11,"HARINA PAN","","CO","harinapan")
	db.Exec(query,12,"MASA DE BUÑUELOS","","CO","bunuelos")
	db.Exec(query,13,"PAN DE BONO","","CO","pandebono")
	db.Exec(query,14,"PANELA","","CO","panela")
	db.Exec(query,15,"PAPAS CRIOLLAS","","CO","papascriollas")
	db.Exec(query,16,"PONY MALTA","","CO","ponymalta")
	db.Exec(query,17,"PULPAS DE FRUTA","","CO","pulpafruta")
	db.Exec(query,18,"RON MEDELLIN","","CO","ronmedellin")
	db.Exec(query,19,"SANCOCHO CONGELADO","","CO","sancochocongelado")
	db.Exec(query,20,"TAJADAS DE PLATANO","","CO","tajadasplatano")
	db.Exec(query,21,"TOSTONES","","CO","tostones")
	db.Exec(query,22,"TRIGUISAR","","CO","triguisar")
	db.Exec(query,22,"CHOCORRAMO","","CO","chocorramo")
}
